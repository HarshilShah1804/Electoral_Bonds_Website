...
def a_2():
    ...
    if len(data)==0:
        return render_template('e_1.html',a_2_data = [['Not Found']])
    print(data)
    return render_template("e_1.html", a_2_data = data,bond_number=bond_number,table_name=table_name) 
...
def e_2():
    if request.method == "POST":
        ...
        cursor.close()
    if len(data)==0:
        return render_template('e_2.html',e_2_data = [['Not Found']])
    print(d)
    years=list(d.keys())
...
def e_3():
    ...
    amount=list(d.values())
    if len(data)==0:
        return render_template('e_3.html',e_3_data = [['Not Found']])
    return render_template('e_3.html',e_3_data = data,years1=years,amount1=amount,party=request.form['party'])
...
def e_4():
    if request.method=="POST":
        ...
        print(data)
        if len(data)==0:
            return render_template('e_4.html',e_4_data = [['Not Found']])
        d={}
        for i in range(len(data)):
...
def e_5():
    if request.method=="POST":
        ...
        data=cursor.fetchall()
        if len(data)==0:
            return render_template('e_5.html',e_5_data = [['Not Found']])
        d={}
        for i in range(len(data)):
